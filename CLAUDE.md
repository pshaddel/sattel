...
`src/db/` — project-local SQLite storage (`bun:sqlite`) at `.sattel/sattel.db`. `db.ts`'s `openDb(cwd)` creates the file/directory and applies any pending migrations before returning the handle; callers own it and must `.close()` it. `migrations/` holds one file per schema change (`NNN-description.ts`, each exporting a `{ id, sql }` literal whose `sql` uses `CREATE ... IF NOT EXISTS` so it's safe to run once and forgotten), listed in order in `migrations/index.ts` and tracked by applied `id` in a `schema_migrations` table so each one runs exactly once. `file.db.ts` is the first table built on this — `mentioned_files` (`id`, `path` UNIQUE, `attempts`, `last_attempt_date`) backs the `@`-mention palette's recently-used file list: `recordFileMention(path, cwd?)` upserts a mention (incrementing `attempts`/refreshing `last_attempt_date` on a repeat, keyed on the `path` UNIQUE constraint) then prunes entries with `last_attempt_date` older than 7 days and caps the table at 200 rows, oldest by `last_attempt_date` dropped first — so re-mentioning a file refreshes its recency and rescues it from eviction, giving true LRU behavior rather than insertion-order truncation; `getRecentFiles(limit?, cwd?)` returns up to `limit` paths ordered most-recent-first, applying the same 7-day cutoff independently at read time (so a stale row still sitting in the table between writes is excluded). Both functions swallow all storage errors (missing dir, locked file, corrupt DB) and fail to a no-op / empty array, since this is a nice-to-have for the mention palette, not something that should ever break the CLI. Wired into `index.ts`: a bare `@` with no query yet renders `getRecentFiles()` in the palette (falling back to the alphabetical project-files list when there's no mention history yet), and selecting a file via Tab or Enter calls `recordFileMention`.
